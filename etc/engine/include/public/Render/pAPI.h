...
namespace PAPI {
    ...
    public:
        ...
        void SourceUniform(const float particle_rate, ///< how many particles to add per unit time
			const float chaos, ///< scatters particle generation time [0..1]
            const pDomain &dom ///< particle positions are chosen from this domain
            );
    ...
    class  ParticleContext_t : public PContextActionList_t, public PContextParticleGroup_t, 
      public PContextActions_t, public PContextSourceState_t
    {
    public:
      /// The context's default constructor.
      ParticleContext_t();
      ~ParticleContext_t();
      PInternalState_t *m_PSt;

	  void TimeStep(float dt)
	  {
		PContextSourceState_t::TimeStep(dt);
		PContextActionList_t::TimeStep(dt);
	  }
	};
};
