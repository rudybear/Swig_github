...
FORCEINLINE int _InterlockedDecrement(int volatile* value)
{
    return OSAtomicDecrement32(value);
    
}

FORCEINLINE int _InterlockedIncrement(int volatile* value)
{
...
FORCEINLINE int _InterlockedExchangeAdd(int volatile* value , int inc)
{
    // FIXME:!!!!!!!!!!!!!!!!!! OSCompareAndSwap
	int oldValue = *value;
	//bool res = 
    OSAtomicCompareAndSwap32(oldValue, oldValue + inc, value);
	
	return oldValue;//
    
}

FORCEINLINE int _InterlockedCompareExchange(int volatile* destination , int exchange, int comparand)
{
	int dest = (int)*destination;
	//bool res = 
    OSAtomicCompareAndSwap32(comparand, exchange, destination );
...
FORCEINLINE void strlwr(char * string)
{
  if (string)
  {
    while (*string != '\0') {
      *string = (unsigned char)tolower(*string);
      ++string;
    }
  }
}
...
extern bool FixPathForPlatform(char * buf, int sizebuf, const char * srcPath, const char * _Mode); 
...
namespace Engine2
{
  namespace Platform
  {
    FORCEINLINE errno_t  fopen_s( FILE ** _File,  const char * _Filename,  const char * _Mode)
    {
      // FIXME:!!!
      char tmpbuf[1024];
      bool bRes = FixPathForPlatform(tmpbuf, 1024, _Filename, _Mode);
      if (!bRes)
        return -1;
      *_File = fopen(tmpbuf, _Mode);
      return *_File == 0 ? -1 : 0; 
    }
