...
{
  __gnu_cxx::__atomic_add_dispatch((_Atomic_word*)value, -1);
  return *value;  
}
...
FORCEINLINE long _InterlockedExchangeAdd(int volatile* value , int inc)
{
  ...
  __gnu_cxx::__atomic_add_dispatch((_Atomic_word*)value, inc);
  return oldValue;  
}
...
FORCEINLINE int __atomic_cmpxchg(int old, int _new, volatile int *ptr)
{
  int result;
...
FORCEINLINE long _InterlockedCompareExchange(long volatile* destination , long exchange, long comparand)
{
  long ret = *destination;
	volatile int * dest = (volatile int *)destination;
  __atomic_cmpxchg(comparand, exchange, dest);
  //atomic_cmpset_ptr((volatile u_int*)destination, comparand, exchange);
  return ret;
}
...
{
    if (value) {
        long unsigned int res = ffs(value);
        *out = res - 1;
        return true;
    }
...
//((void)__android_log_print(ANDROID_LOG_INFO, "engine2", __VA_ARGS__))
